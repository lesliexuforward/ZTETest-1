...
public class MySymbolGraph {
	...
	private MyEdgeWeightedGraph graph; // the underlying graph
	...
	public MySymbolGraph(File filename, String delimiter) {
        ...
        // 将数据读入图的邻接表
        graph = new MyEdgeWeightedGraph(st.size());
        in = new In(filename);
	...
	//返回无向图
	public MyEdgeWeightedGraph graph() {
		return graph;
	}
	...
	public String toString() {
		...
		s.append(st.size() + " " + graph.E() + MyEdgeWeightedGraph.NEWLINE);
		for (int v = 0; v < st.size(); v++) {
			s.append(keys[v] + ": ");
			for (MyEdge e : graph.getAdj()[v]) {
				s.append(keys[e.getV()] + " -- "+keys[e.getW()]+" "+e.getWeight()+" ");
			}
			s.append(MyEdgeWeightedGraph.NEWLINE);
		}
		return s.toString();
	}
