...
public class MyEdgeWeightedDigraph {
	...
	public MyEdgeWeightedDigraph(MyEdgeWeightedDigraph G) {
		...
		for (int v = 0; v < G.V(); v++) {
			...
			Stack<MyDirectedEdge> reverse = new Stack<MyDirectedEdge>();
			for (MyDirectedEdge e : G.adj[v]) {
				reverse.push(e);
			}
			for (MyDirectedEdge e : reverse) {
				adj[v].add(e);
			}
		}
	}
	 ...
	 * {@code for (MyDirectedEdge e : G.edges())}.
	 *
	...
	/**
	 * Unit tests the {@code MyEdgeWeightedDigraph} data type.
	 *
	 * @param args
